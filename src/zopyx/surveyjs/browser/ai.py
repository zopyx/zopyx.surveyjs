...
- CRITICAL - Row grouping using geometry: Field geometry includes `row_y` (vertical position identifier for row grouping), `x` (horizontal position), and `page` (page number). Fields on the same page with the same `row_y` value belong on the same visual row. For each unique combination of page and row_y, create a SurveyJS element with `type: "panel"` and nest all fields sharing that row_y as nested `elements` inside that panel. Within each panel, sort the nested elements by their `x` coordinate (left-to-right) to maintain correct field order. This creates a proper row-based layout where each row is represented as a panel containing its nested field elements.
