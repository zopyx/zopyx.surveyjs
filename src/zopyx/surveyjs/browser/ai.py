...
- CRITICAL - Row grouping using geometry: Field geometry includes `row_y` (vertical position identifier for row grouping) and `x` (horizontal position). Fields that appear on the same visual row in the PDF have the same `row_y` value. Group fields into rows by their `row_y` value - all fields sharing the same `row_y` belong in the same row. Within each row, sort fields by their `x` coordinate (left-to-right) to maintain the correct field order. Use this row-based structure when creating SurveyJS layout (e.g., place fields in the same `panel` or use `startWithNewLine: false` for fields on the same row).
