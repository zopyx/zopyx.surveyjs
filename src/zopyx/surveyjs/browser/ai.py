...
- CRITICAL - Row grouping using geometry: Field geometry includes `normalized_y` (0-1 range, vertical position on page) and `x` (horizontal position). Fields that appear on the same visual row in the PDF typically have very similar `normalized_y` values (within a small tolerance of ~0.01-0.02). Group fields into rows by clustering them based on similar `normalized_y` values. Within each row, sort fields by their `x` coordinate (left-to-right) to maintain the correct field order. Use this row-based structure when creating SurveyJS layout (e.g., place fields in the same `panel` or use `startWithNewLine: false` for fields on the same row).
