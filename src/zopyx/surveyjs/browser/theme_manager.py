...
class ThemeManagerView(Views):
    ...
    def _upload_theme(self):
        ...
        if not isinstance(theme_json, dict) or "themeName" not in theme_json:
            ...
            return json.dumps(
                {
                    "success": False,
                    "error": "Invalid theme JSON: missing 'themeName' property",
                }
            )
        if not name:
        ...
        theme = themes_service.create_theme(
            self._annotations, name, theme_json, user_id=user
        )
        import transaction
    ...
    def _download_theme(self):
        theme_id = (
            self.request.get("theme_id")
            or (self.request.form.get("theme_id") or "").strip()
        )
