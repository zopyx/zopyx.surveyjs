...
class ResultServiceTests(unittest.TestCase):
    ...
    def test_parse_tabulator_param_handles_empty_lists_and_invalid_json(self):
        ...
        self.assertEqual(
            results.parse_tabulator_param(
                self.request(filters=['[{"field": "user"}]']), "filters"
            ),
            [{"field": "user"}],
        )
        self.assertEqual(
    ...
    def test_results_apply_filters_supports_comparisons_and_in(self):
        ...
        self.assertEqual(
            len(
                results.results_apply_filters(
                    rows, [{"field": "user", "type": "=", "value": "Alice"}]
                )
            ),
            1,
        )
        self.assertEqual(
            len(
                results.results_apply_filters(
                    rows, [{"field": "created_ts", "type": ">=", "value": 20}]
                )
            ),
            1,
        )
        self.assertEqual(
            len(
                results.results_apply_filters(
                    rows, [{"field": "user", "type": "in", "value": ["Bob"]}]
                )
            ),
            1,
        )
        self.assertEqual(
            len(
                results.results_apply_filters(
                    rows, [{"field": "user", "type": "contains", "value": "ali"}]
                )
            ),
            1,
        )
        self.assertEqual(
            len(
                results.results_apply_filters(
                    rows, [{"field": "user", "type": "unknown", "value": "x"}]
                )
            ),
            2,
        )
    ...
    def test_build_results_payload_clamps_invalid_pagination_and_accepts_dict_params(
        self,
    ):
        payload = results.build_results_payload(
            [],
    ...
    def test_get_paginated_results_searches_by_uuid_and_paginates(self):
        ...
        payload = results.get_paginated_results(
            data, self.request(q="target", b_start="0")
        )
