...
class BrowserCoverageViewTests(unittest.TestCase):
    ...
    def test_chatbot_payload_context_and_json_field_coercion(self):
        ...
        with (
            patch.object(chatbot_module, "parse_json_body", return_value=None),
            patch("plone.api.user.get_roles", return_value=["Editor"]),
        ):
        ...
        self.assertEqual(
            view._coerce_json_field("invalid", {"fallback": True}), {"fallback": True}
        )
    ...
    def test_chatbot_api_guards_stream_success_and_failure(self):
        ...
        with (
            ...
            patch.object(view, "_chatbot_enabled", return_value=False),
        ...
        with (
            patch.object(view, "_check_post_authenticator"),
            patch.object(view, "_chatbot_enabled", return_value=True),
            patch.object(
                type(view),
        ...
        with (
            ...
            patch.object(view, "_request_payload", return_value={}),
            patch.object(chatbot_module, "json_error") as error,
        ):
            view.chatbot_api()
        ...
        with (
            ...
            patch.object(
                view,
                "_request_payload",
                return_value={"message": "hi", "stream": "true"},
            ),
            patch.object(view, "_ensure_local_index"),
            patch.object(view, "_build_context"),
            patch.object(view, "_engine", return_value=engine),
        ):
            view.chatbot_api()
        ...
        with (
            ...
            patch.object(view, "_request_payload", return_value={"message": "hi"}),
            patch.object(view, "_ensure_local_index"),
            patch.object(view, "_build_context"),
            patch.object(view, "_engine", return_value=engine),
            patch.object(chatbot_module, "json_error") as error,
    ...
    def test_chatbot_management_index_remote_and_reset(self):
        ...
        with (
            ...
            patch.object(view, "_store") as store,
            patch.object(chatbot_module, "json_response") as response,
        ):
        ...
        with (
            ...
            patch.object(view, "_indexer") as indexer,
            patch.object(chatbot_module, "json_response"),
        ):
    ...
    def test_monitor_json_cleanup_chart_and_empty_data(self):
        ...
        stats = {
            "time_series": {"a": 2, "b": 3},
            "form_time_series": [
                {"title": "Form", "path": "/f", "count": 5, "series": {"a": 1}}
            ],
    ...
    def test_monitor_helpers_rate_limit_and_json_safe(self):
        ...
        view.time_window = "1h"
        with patch.object(
            monitor_module, "check_rate_limit", return_value=(False, {"allowed": False})
        ):
            self.assertEqual(view.get_rate_limit_status()["allowed"], False)
        with (
            patch.object(
                monitor_module, "get_submission_stats", return_value={"time_series": {}}
            ),
            patch(
                "zopyx.surveyjs.browser.survey_monitor.html_safe_json",
                return_value="{}",
            ) as safe,
        ):
    ...
    def test_psf_properties_templates_and_creation_errors(self):
        ...
        with (
            ...
            patch.object(view, "_handle_create_from_template", return_value="x"),
        ):
    ...
    def test_embed_token_and_asset_views_cover_guards(self):
        ...
        with (
            ...
            patch.object(
                embed_module,
                "parse_json_body",
                return_value={"origin": "https://bad.example"},
            ),
            patch.object(embed_module, "json_error") as error,
        ):
            view()
        ...
        asset.request.get.side_effect = (
            lambda key, default="": "wrong.js" if key == "name" else default
        )
    ...
    def test_embed_config_rejects_disabled_origin_missing_and_bad_token(self):
        ...
        with (
            ...
            patch.object(embed_module, "handle_cors_preflight", return_value=False),
            patch.object(
                view.request,
                "get_header",
                side_effect=lambda name: "https://app.example"
                if name == "Origin"
                else None,
            ),
            patch.object(
        ...
        view.request.get_header.side_effect = (
            lambda name: "https://app.example" if name == "Origin" else "token"
        )
        with (
            patch.object(
                embed_module, "is_embed_direct_globally_enabled", return_value=True
            ...
            patch.object(
                ...
                side_effect=embed_module.TokenInvalidError("bad"),
            ),
            patch.object(embed_module, "json_error") as error,
        ):
            view()
