...
class ConfiguredKVStoreTests(unittest.TestCase):
    def test_diskcache_configuration_is_instance_home_relative_and_namespaced(self):
        with TemporaryDirectory() as instance_home:
            settings = SimpleNamespace(
                kv_cache_backend="diskcache",
                kv_cache_directory="var/surveyjs-cache",
                kv_cache_lock_timeout_seconds=2,
            )
            with patch.dict(os.environ, {"INSTANCE_HOME": instance_home}):
                ...
                self.assertEqual(list(store.iterkeys()), ["received:token"])
                self.assertTrue(
    ...
    def test_rdbms_configuration_requires_explicit_uri(self):
        settings = SimpleNamespace(
            kv_cache_backend="rdbms",
            kv_cache_database_uri="",
            database_uri="sqlite:///var/results.db",
        )
        with self.assertRaisesRegex(ValueError, "kv_cache_database_uri"):
            get_configured_kv_store(settings, "monitoring")

    def test_rdbms_configuration_requires_uri(self):
        settings = SimpleNamespace(kv_cache_backend="rdbms", database_uri="")
        with self.assertRaisesRegex(ValueError, "kv_cache_database_uri"):
            get_configured_kv_store(settings, "auth")

    def test_expired_cleanup_is_bounded_and_counted(self):
        with TemporaryDirectory() as td:
            store = SQLKVStore(f"sqlite:///{td}/kv.db")
            self.addCleanup(store.close)
            store.set("expired-1", "v", expire=-1)
            store.set("expired-2", "v", expire=-1)
            store.set("live", "v", expire=60)
            before = get_kv_metrics().get("expired_entries_removed", 0)
            self.assertEqual(store.cleanup_expired(limit=1), 1)
            self.assertEqual(store.get("live"), "v")
            self.assertEqual(
                get_kv_metrics().get("expired_entries_removed", 0), before + 1
            )

    def test_remote_postgresql_requires_tls(self):
        with self.assertRaisesRegex(ValueError, "sslmode"):
            validate_kv_database_uri("postgresql+psycopg2://user:pass@dbhost/db")
        url = validate_kv_database_uri(
            "postgresql+psycopg2://user:pass@dbhost/db?sslmode=verify-full"
        )
        self.assertEqual(url.get_backend_name(), "postgresql")

    def test_remote_mysql_requires_tls_and_supported_driver(self):
        with self.assertRaisesRegex(ValueError, "TLS"):
            validate_kv_database_uri("mysql+pymysql://user:pass@dbhost/db")
        url = validate_kv_database_uri(
            "mysql+pymysql://user:pass@dbhost/db?ssl_ca=/etc/ssl/ca.pem"
        )
        self.assertEqual(url.drivername, "mysql+pymysql")
        with self.assertRaisesRegex(ValueError, "driver"):
            validate_kv_database_uri("mysql+asyncmy://user:pass@localhost/db")

    def test_diagnostics_exclude_database_password(self):
        settings = SimpleNamespace(
            kv_cache_backend="rdbms",
            kv_cache_database_uri="postgresql+psycopg2://user:secret@localhost/db",
        )
