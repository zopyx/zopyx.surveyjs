...
Results are available via `@@results` when the `store` action is
enabled. Each submission can be inspected, exported, emailed, or POSTed
to an endpoint.
...
The result detail view (`@@result-detail`) renders a human-readable view
of a submission using the form schema to map labels to values.
